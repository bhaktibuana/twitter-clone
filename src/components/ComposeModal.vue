<template>
  <div class="backdrop" :class="{ show: isVisible }" @click.self="handleClose">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleComposeHeight = () => {
      compose.value.style.height = "100px";
    ...
    watch(userData, () => {
      userImage.value = imageUrl.value;
    });
...
.compose-modal {
  display: flex;
  width: 100%;
  max-width: var(--LEFT_CONTENT_WIDTH);
  height: 100vh;
  margin: auto;
